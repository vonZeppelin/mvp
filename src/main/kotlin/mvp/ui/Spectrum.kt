...
class Spectrum : Canvas() {
    init {
        val spectrumService = object : ScheduledService<DoubleArray>() {
        ...
        Player.statusProperty.addListener { _, oldValue, newValue ->
            when {
                newValue == Player.Status.PLAYING -> spectrumService.restart()
                oldValue == Player.Status.PLAYING -> spectrumService.cancel()
            }
        }
    }
    ...
    private fun drawNoSound() {
        drawBands(DoubleArray(BANDS_COUNT) { 1.0 })
    }

    private fun drawBands(bands: DoubleArray) {
